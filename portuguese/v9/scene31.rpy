# TODO: Translation updated at 2021-05-24 07:15

# game/v9/scene31.rpy:22
translate portuguese v9_sat_cafe_w_nora_2476391f:

    # no "It feels good to have a nice meal once in awhile."
    no "É bom ter uma boa refeição de vez em quando."

# game/v9/scene31.rpy:27
translate portuguese v9_sat_cafe_w_nora_67cd3695:

    # u "Do you and Chris not go on dates now and then?"
    u "Você e Chris não saem em encontros de vez em quando?"

# game/v9/scene31.rpy:32
translate portuguese v9_sat_cafe_w_nora_ac079442:

    # no "Do you really have to ask? We spent a lot of time together when we first started dating, but now it's like I'm second to the Wolves."
    no "Você acha que precisa perguntar isso? A gente passava muito tempo juntos quando começamos a namorar, mas agora é como se eu estivesse em segundo lugar em relação aos Lobos."

# game/v9/scene31.rpy:33
translate portuguese v9_sat_cafe_w_nora_1264b6df:

    # no "And I know I shouldn't be jealous, but in a way I am. Like, if you have all this passion, love and attention to give, can some come my way?"
    no "E eu sei que não deveria ter ciúmes, mas de certa forma eu tenho. Tipo, se você tem toda essa paixão, amor e atenção pra dar, \nserá que um pouco disso não pode vir em pra mim?"

# game/v9/scene31.rpy:38
translate portuguese v9_sat_cafe_w_nora_f7ecc16b:

    # u "I think you're right to see it the way you do. Your partner should come first regardless, and even though all of this seems important now, it's all temporary."
    u "Acho que você está certa em ver as coisas assim. \nSeu parceiro deve vir em primeiro lugar, e mesmo que tudo isso \npareça importante agora, tudo isso é temporário."

# game/v9/scene31.rpy:43
translate portuguese v9_sat_cafe_w_nora_dd3f3a9e:

    # no "Wow, under that thick skull you're not as dull as the rest of those meat heads."
    no "Uau, sob esse crânio grosso você não é tão insensível quanto o resto desses bocós."

# game/v9/scene31.rpy:48
translate portuguese v9_sat_cafe_w_nora_c8e4e4ad:

    # u "Haha, surprise!"
    u "Haha, surpresa!"

# game/v9/scene31.rpy:53
translate portuguese v9_sat_cafe_w_nora_1ab526a2:

    # waiter "Good morning, what can I get for the couple?"
    waiter "Bom dia, o que posso trazer para o casal?"

# game/v9/scene31.rpy:62
translate portuguese v9_sat_cafe_w_nora_44c25675:

    # u "Actually, we're just friends."
    u "Na verdade, somos apenas amigos."

# game/v9/scene31.rpy:67
translate portuguese v9_sat_cafe_w_nora_3c8f81cb:

    # waiter "Ahhh okay, what will you be having today?"
    waiter "Ahhh tá, é que pareciam ser, \nmas então, o que vocês vão querer hoje?"

# game/v9/scene31.rpy:72
translate portuguese v9_sat_cafe_w_nora_2027a26c:

    # u "Any good news about Europe?"
    u "Alguma boa notícia sobre a viagem a Europa?"

# game/v9/scene31.rpy:78
translate portuguese v9_sat_cafe_w_nora_3bfd1c84:

    # no "It's going great! We almost have everything we need. I'm thinking about seeing if we can get a bit more involvement from the houses. I mentioned it to Chris, but he only said \"we'll get to that later\". It means a lot to me that you've taken an interest."
    no "Está indo muito bem! Temos quase tudo o que precisamos. \nEstou pensando em ver se podemos ter um pouco mais de envolvimento das fraternidades. Mencionei isso ao Chris, mas ele só disse \"vamos resolver a isso mais tarde\". Significa muito que você tenha se interessado."

# game/v9/scene31.rpy:81
translate portuguese v9_sat_cafe_w_nora_6208327c:

    # no "It's going pretty good. Checking something off the list everyday."
    no "Está indo muito bem. Completando algo da lista todos os dias."

# game/v9/scene31.rpy:86
translate portuguese v9_sat_cafe_w_nora_97e99bbf:

    # u "Well I'm glad to hear it's all coming together."
    u "Bem, fico feliz em saber que tudo está se encaixando."

# game/v9/scene31.rpy:91
translate portuguese v9_sat_cafe_w_nora_20c7b874:

    # no "Thanks for asking."
    no "Obrigada por perguntar."

# game/v9/scene31.rpy:101
translate portuguese v9_sat_cafe_w_nora_f0c7c13c:

    # no "I had a really good time [name]. Wish I could do this more often."
    no "Eu aproveitei bastante [name]. \nGostaria de repetir isso em outras ocasiões."

# game/v9/scene31.rpy:106
translate portuguese v9_sat_cafe_w_nora_8c6198f5:

    # u "Maybe we can."
    u "Pode ser."

# game/v9/scene31.rpy:111
translate portuguese v9_sat_cafe_w_nora_44c2578a:

    # no "Maybe. See you around!"
    no "Talvez. Vejo você por aí!"

# game/v9/scene31.rpy:116
translate portuguese v9_sat_cafe_w_nora_18647189:

    # u "See ya!"
    u "Até a próxima!"

# game/v9/scene31.rpy:127
translate portuguese v9_sat_cafe_w_nora_2027a26c_1:

    # u "Any good news about Europe?"
    u "Alguma boa notícia sobre a Europa?"

# game/v9/scene31.rpy:133
translate portuguese v9_sat_cafe_w_nora_76bba1c4:

    # no "It's going great! We almost have everything we need. I'm thinking about seeing if we can get a bit more involvement from the houses."
    no "Está indo muito bem! Temos quase tudo o que precisamos. \nEstou pensando em ver se podemos ter um pouco mais de envolvimento das fraternidades."

# game/v9/scene31.rpy:134
translate portuguese v9_sat_cafe_w_nora_3576a6fd:

    # no "I mentioned it to Chris, but he only said \"we'll get to that later\". It means a lot to me that you've taken an interest."
    no "Mencionei isso ao Chris, mas ele só disse \"vamos resolver a isso mais tarde\". Significa muito que você tenha mostrado interesse."

# game/v9/scene31.rpy:137
translate portuguese v9_sat_cafe_w_nora_6208327c_1:

    # no "It's going pretty good. Checking something off the list everyday."
    no "Está indo muito bem. Completando algo da lista todos os dias."

# game/v9/scene31.rpy:142
translate portuguese v9_sat_cafe_w_nora_97e99bbf_1:

    # u "Well I'm glad to hear it's all coming together."
    u "Bom, fico feliz em saber que tudo está se encaixando."

# game/v9/scene31.rpy:147
translate portuguese v9_sat_cafe_w_nora_20c7b874_1:

    # no "Thanks for asking."
    no "Obrigada por perguntar."

# game/v9/scene31.rpy:157
translate portuguese v9_sat_cafe_w_nora_f0c7c13c_1:

    # no "I had a really good time [name]. Wish I could do this more often."
    no "Eu aproveitei bastante hoje [name]. Gostaria de poder fazer isso mais vezes."

# game/v9/scene31.rpy:162
translate portuguese v9_sat_cafe_w_nora_8c6198f5_1:

    # u "Maybe we can."
    u "Talvez possamos."

# game/v9/scene31.rpy:167
translate portuguese v9_sat_cafe_w_nora_44c2578a_1:

    # no "Maybe. See you around!"
    no "Talvez. Vejo você por aí!"

# game/v9/scene31.rpy:172
translate portuguese v9_sat_cafe_w_nora_18647189_1:

    # u "See ya!"
    u "Até a próxima!"

# game/v9/scene31.rpy:183
translate portuguese v9_sat_cafe_e37871c5:

    # u "(I actually can't believe how hungry I am.)"
    u "(Na verdade, eu nem sabia que podia ter tanta fome.)."

# game/v9/scene31.rpy:188
translate portuguese v9_sat_cafe_c68fb736:

    # u "(What the fuck!?)"
    u "(Mas que porra!?)"

# game/v9/scene31.rpy:197
translate portuguese v9_sat_cafe_68ef14fe:

    # unknown "Hey, sorry but you're the guy that punched that other guy on Kiwii right?"
    unknown "Ei, desculpa, mas você é o cara que esmurrou aquele \ncara naquele vídeo do Kiwii, né?"

# game/v9/scene31.rpy:202
translate portuguese v9_sat_cafe_7720a2b5:

    # u "Uh, yeah that's me."
    u "Ah, sim, sou eu."

# game/v9/scene31.rpy:207
translate portuguese v9_sat_cafe_cfecf5c5:

    # unknown "Wow, I can't believe it's you. Like you're actually here in front of me right now."
    unknown "Uau, não acredito que seja você. \nTipo você está literalmente aqui, \nna minha frente, neste momento."

# game/v9/scene31.rpy:212
translate portuguese v9_sat_cafe_8eb7f5ad:

    # u "Thanks..."
    u "Valeu..."

# game/v9/scene31.rpy:217
translate portuguese v9_sat_cafe_a061b1dc:

    # unknown "Can I sit with you or is that too much? No that's too much, or is it? OMG, you're so stupid. Not you, me. OMG!"
    unknown "Posso sentar com você ou isso é demais? Não, isso é demais, \nou será que não é ? Meu Deus, você é tão estúpida. Não você, eu. Meu Deus!"

# game/v9/scene31.rpy:222
translate portuguese v9_sat_cafe_d653b00c:

    # u "(Okay...)"
    u "(Está bem...)"

# game/v9/scene31.rpy:228
translate portuguese v9_sat_cafe_8b513a1b:

    # unknown "You're the guy that got his ass kicked on Kiwii right?"
    unknown "Você é o cara que levou uma surra \nque está naquele vídeo do Kiwii, né?"

# game/v9/scene31.rpy:233
translate portuguese v9_sat_cafe_9678d4dc:

    # u "Well I wouldn't say I got my ass kicked, but yeah."
    u "Bem, eu não diria que levei uma surra, mas sim."

# game/v9/scene31.rpy:238
translate portuguese v9_sat_cafe_cb76e0dd:

    # unknown "Well everyone else would call it getting your ass kicked, including me."
    unknown "Bem, todo mundo chamaria isso de ter levado uma surra, inclusive eu."

# game/v9/scene31.rpy:243
translate portuguese v9_sat_cafe_67e48a73:

    # u "(Okay... bitch.)"
    u "(Certo... piranha...)"

# game/v9/scene31.rpy:248
translate portuguese v9_sat_cafe_cf7579f5:

    # u "(Well, let the crazy begin.)"
    u "(Bem, que comece a loucura)."

translate portuguese strings:

    # game/v9/scene31.rpy:58
    old "Correct Waiter"
    new "Corregir a garçonete"

    # game/v9/scene31.rpy:58
    old "Don't correct Waiter"
    new "Não corrija a garçonete."
# TODO: Translation updated at 2021-07-31 16:17

# game/v9/scene31.rpy:32
translate portuguese v9_sat_cafe_w_nora_92966436:

    # no "Do you really have to ask? We spent a lot of time together when we first started dating, but now it's like I'm second to the Wolves. And I know I shouldn't be jealous, but in a way I am. Like, if you have all this passion, love and attention to give, can some come my way?"
    no "Você realmente precisa perguntar? A gente passava muito tempo juntos quando começamos a namorar, mas agora parece que eu fiquei em segundo lugar. Eu sei que não deveria ficar ciumenta, mas de certo modo eu estou. Tipo, se você consegue ter tanto amor e paixão por algo, talvez poderia dar um pouco pra mim?"

# game/v9/scene31.rpy:132
translate portuguese v9_sat_cafe_w_nora_3bfd1c84_1:

    # no "It's going great! We almost have everything we need. I'm thinking about seeing if we can get a bit more involvement from the houses. I mentioned it to Chris, but he only said \"we'll get to that later\". It means a lot to me that you've taken an interest."
    no "Está tudo ótimo! Já fizemos quase tudo que precisávamos. Tô pensando em ver se a gente consegue mais envolvimento por parte das fraternidade. Eu mencionei pro Chris, mas ele só disse \"a gente vê isso depois\". Significa muito que você tenha tido interesse."

translate portuguese strings:

    # game/v9/scene31.rpy:57
    old "Don't Correct Waiter"
    new "Não corrigir a garçonete."

