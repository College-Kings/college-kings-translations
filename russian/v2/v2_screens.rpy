# TODO: Translation updated at 2023-06-23 14:33
# Перевод сделан: Anukich#0751 (discord)

translate russian strings:

    # game/v2/v2_screens.rpy:35
    old "Once you decide which outfit to buy, you will no longer be able to play through all of the \"Try\" scenes. Are you sure you want to continue?"
    new "Как только вы решите, какой наряд купить, вы больше не сможете сыграть во все остальные сцены \"Примерки\". Вы уверены, что хотите продолжить?"

