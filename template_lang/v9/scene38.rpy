# TODO: Translation updated at 2021-05-24 07:15

# game/v9/scene38.rpy:21
translate template_lang v9_walk_li_txt_1d8a7ae7:

    # u "(Ugh, this is such a mess. How did I get stuck in the middle of those two?)"
    u "(Ugh, que confusão. Como é que eu fiquei preso no meio daqueles dois?)"

# game/v9/scene38.rpy:23
translate template_lang v9_walk_li_txt_ed398106:

    # u "(I may need some help if they run into each other again)"
    u "(Posso precisar de ajuda se eles se encontrarem novamente)"

# game/v9/scene38.rpy:32
translate template_lang v9_walk_li_txt_112e72aa:

    # u "(Please no more drama)"
    u "(Por favor, sem mais drama)"

# game/v9/scene38.rpy:46
translate template_lang s38_PhoneContinue_73e9c274:

    # "(I should reply to Lindsey.)"
    "(Eu deveria responder a Lindsey)."

# game/v9/scene38.rpy:53
translate template_lang s38_PhoneContinue_744c4169:

    # u "(I suppose I should head to see Lindsey.)"
    u "(Melhor eu ir ver a Lindsey agora)."

# game/v9/scene38.rpy:60
translate template_lang s38_PhoneContinue_538b62a3:

    # u "(Shit.)"
    u "(Porra.)"

translate template_lang strings:

    # game/v9/scene38.rpy:8
    old "Great. See ya soon"
    new "Ótimo. Nos vemos em breve"

    # game/v9/scene38.rpy:12
    old ":("
    new ":("

    # game/v9/scene38.rpy:13
    old "Please don't hate me. You know I want to. I really, really want to."
    new "Por favor, não me odeie. Você sabe que eu quero. Eu realmente, realmente quero estar aí."

    # game/v9/scene38.rpy:14
    old "No hard feelings. Bye"
    new "Sem ressentimentos. Tchau"

    # game/v9/scene38.rpy:15
    old "Maybe after the fight?"
    new "Talvez depois da briga?"

    # game/v9/scene38.rpy:34
    old "Hey Freshmeat! How's it hangin?"
    new "Ei, carne fresca! Como está indo?"

    # game/v9/scene38.rpy:35
    old "Hey Linds ;)"
    new "Hey Linds ;)"

    # game/v9/scene38.rpy:36
    old "I've decided to allow the nickname under one condition"
    new "Eu decidi deixar você usar esse apelido sob uma condição."

    # game/v9/scene38.rpy:37
    old "Really? And what's that?"
    new "Sério? E o que eu vou fazer?"

    # game/v9/scene38.rpy:38
    old "Come hang out with me"
    new "Vem aqui encontrar comigo."

    # game/v9/scene38.rpy:39
    old "Hell yeah! Be right there, Linds!"
    new "Isso é um prazer! Já estou indo, Linds!"

    # game/v9/scene38.rpy:40
    old "Aww, I wish I could, but I gotta get ready for the brawl"
    new "Aww, eu gostaria poderir agora, mas tenho que me preparar para a briga."

    
