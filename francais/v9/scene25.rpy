# TODO: Translation updated at 2021-11-28 21:19

# game/v9/scene25.rpy:11
translate francais v9_room_fri_eve_303d2d04:

    # u "(I wonder how much Mr. Lee expects us to remember from everyone else's Viking scenes. I was too busy looking at the costumes to pay attention.)"
    u "(Je me demande à quoi s'attend Mr Lee, sur le fait que nous nous souvenions des scènes de Vikings des autres. J'étais trop occupé à regarder les costumes pour y prêter attention.)"

# game/v9/scene25.rpy:18
translate francais v9_room_fri_eve_5ef4f2a2:

    # u "(I'm so bored.)"
    u "(Je m'ennuie tellement.)"

# game/v9/scene25.rpy:24
translate francais v9_room_fri_eve_e5648356:

    # u "(Wonder how Lauren's doing with the Deers charity.)"
    u "(Je me demande comment Lauren s'en sort avec la fête de charité des Deers.)"

# game/v9/scene25.rpy:38
translate francais s25_ContinueW_0f06666e:

    # "(I should text Lauren.)"
    "(Je devrais envoyer un message à Lauren.)"

# game/v9/scene25.rpy:46
translate francais s25_ContinueW_4830ad53:

    # "*Knock* *knock*"
    "*Toc* *Toc*"

# game/v9/scene25.rpy:48
translate francais s25_ContinueW_477524e5:

    # u "Come in!"
    u "Entrez !"

# game/v9/scene25.rpy:56
translate francais s25_ContinueW_303d2d04:

    # u "(I wonder how much Mr. Lee expects us to remember from everyone else's Viking scenes. I was too busy looking at the costumes to pay attention.)"
    u "(Je me demande combien Mr Lee s'attend à ce que nous nous souvenions des scènes de Vikings de tous les autres. J'étais trop occupé à regarder les costumes pour y prêter attention.)"

# game/v9/scene25.rpy:63
translate francais s25_ContinueW_5ef4f2a2:

    # u "(I'm so bored.)"
    u "(Je m'ennuie tellement.)"

# game/v9/scene25.rpy:69
translate francais s25_ContinueW_e5648356:

    # u "(Wonder how Lauren's doing with the Deers charity.)"
    u "(Je me demande comment Lauren s'en sort avec la fête de charité des Deers.)"

# game/v9/scene25.rpy:82
translate francais s25_ContinueA_0f06666e:

    # "(I should text Lauren.)"
    "(Je devrais envoyer un message à Lauren.)"

# game/v9/scene25.rpy:90
translate francais s25_ContinueA_4830ad53:

    # "*Knock* *knock*"
    "*Toc* *Toc*"

# game/v9/scene25.rpy:92
translate francais s25_ContinueA_477524e5:

    # u "Come in!"
    u "Entrez !"

translate francais strings:

    # game/v9/scene25.rpy:27
    old "How's it going? You still doing our statue idea?"
    new "Comment ça se passe ? Tu réalises toujours notre idée de statue ?"

    # game/v9/scene25.rpy:28
    old "Of course. I'm glad you talked me into it."
    new "Bien sûr. Je suis contente que tu m'aies convaincu."

    # game/v9/scene25.rpy:29
    old "Me too. I don't think anyone's tried something like this before. You'll be the talk of the school!"
    new "Moi aussi. Je ne pense pas que quelqu'un ait déjà essayé quelque chose comme ça avant. Tu seras le sujet de conversation de l'école !"

    # game/v9/scene25.rpy:30
    old "No, WE will. I couldn't have done it without your help."
    new "Non, NOUS le serons. Je n'aurais pas pu le faire sans ton aide."

    # game/v9/scene25.rpy:31
    old "Don't mention it, talk soon?"
    new "Ne dis pas ça, on en parle bientôt ?"

    # game/v9/scene25.rpy:32
    old "Sure!"
    new "Bien sûr !"
