# TODO: Translation updated at 2021-11-28 14:27

# game/v9/scene17.rpy:11
translate francais v9_room_fri_morn_eef583a3:

    # ch "Wake up [name], Wolves aren't nocturnal. Go be productive. And someone sent you some mail Mr. Popular."
    ch "Réveille-toi [name], les Wolves ne sont pas nocturnes. Sois productif. Et quelqu'un t'a envoyé du courrier, Mr Populaire."

# game/v9/scene17.rpy:16
translate francais v9_room_fri_morn_8da0ae81:

    # u "Early bird gets the worm and yada yada yada."
    u "L'oiseau précoce obtient le ver et gna gna gna."

# game/v9/scene17.rpy:21
translate francais v9_room_fri_morn_4471b800:

    # ch "Right!"
    ch "C'est ça !"

# game/v9/scene17.rpy:35
translate francais v9_room_fri_morn_db7a4e2e:

    # u "(Over the semester you've been way more than a student, a true confidant. I hope I'll have the opportunity to return the favor. With gratitude, Miss Rose.)"
    u "(Au cours du semestre, vous avez été bien plus qu'un étudiant, un véritable confident. J'espère que j'aurai l'occasion de vous rendre la pareille. Avec toute ma gratitude, Mme Rose.)"

# game/v9/scene17.rpy:40
translate francais v9_room_fri_morn_f92a2101:

    # u "(Ms. Rose, did she get a divorce? And what does she mean return the favor? Guess I'll find out soon.)"
    u "(Mme Rose, a-t-elle divorcé ? Et que veut-elle dire par rendre la pareille ? Je crois que je vais le découvrir bientôt)."

# game/v9/scene17.rpy:55
translate francais s17_PhoneContinueW_0f06666e:

    # "(I should text Lauren.)"
    "(Je devrais envoyer un message à Lauren.)"

# game/v9/scene17.rpy:58
translate francais s17_PhoneContinueW_23ba1da1:

    # u "(I better get ready and head over there. Hope it's not too much work.)"
    u "(Je ferais mieux de me préparer et d'aller là-bas. J'espère que ce n'est pas trop de travail.)"

# game/v9/scene17.rpy:66
translate francais s17_PhoneContinueW_5cdddfcb:

    # gr "Wake up, [name], Apes aren't lazy assholes. Get up. Go be productive. Oh, and I'd avoid the toilet. Cameron backed it up again."
    gr "Réveille-toi, [name], les Apes ne sont pas des trous du cul paresseux. Lève-toi. Sois productif. Oh, et j'éviterais les toilettes. Cameron les a encore contaminées."

# game/v9/scene17.rpy:71
translate francais s17_PhoneContinueW_19541c97:

    # u "*Drowsy* Uhh, yeah ok. I'm up, and ewww."
    u "*Somnole* Uhh, ouais, ok. Je suis debout, et beuuuuh."

# game/v9/scene17.rpy:76
translate francais s17_PhoneContinueW_68aafa48:

    # gr "Good."
    gr "Bien."

# game/v9/scene17.rpy:81
translate francais s17_PhoneContinueW_347d66bc:

    # u "(Is this guy my fucking dad? Jesus.)"
    u "(Ce gars se prend pour mon putain de père ? Merde.)"

# game/v9/scene17.rpy:96
translate francais s17_PhoneContinueA_0f06666e:

    # "(I should text Lauren.)"
    "(Je devrais envoyer un message à Lauren.)"

# game/v9/scene17.rpy:99
translate francais s17_PhoneContinueA_23ba1da1:

    # u "(I better get ready and head over there. Hope it's not too much work.)"
    u "(Je ferais mieux de me préparer et d'aller là-bas. J'espère que ce n'est pas trop de travail.)"

translate francais strings:

    # game/v9/scene17.rpy:47
    old "Hey, would yould you like to come over to my dorm and help me? I need help with my Deer initiation. They want me to help with the annual charity day event they have."
    new "Hey, tu voudrais bien venir à mon dortoir et m'aider ? J'ai besoin d'aide pour mon initiation aux Deers. Ils veulent que je les aide pour la journée annuelle de charité qu'ils organisent."

    # game/v9/scene17.rpy:48
    old "Sure, OMW!"
    new "Bien sûr !"

    # game/v9/scene17.rpy:49
    old "Ok :) See you soon!"
    new "Ok :) A bientôt !"
