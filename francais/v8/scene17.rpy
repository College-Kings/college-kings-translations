# TODO: Translation updated at 2021-06-05 20:30

# game/v8/scene17.rpy:19
translate francais msrose_moving_331a7fab:

    # u "(Should I check how Penelope's doing?)"
    u "(Devrais-je vérifier comment Penelope vas ?)"

# game/v8/scene17.rpy:25
translate francais phn_penelope6_26a5b4a3:

    # u "(I should talk to Penelope.)"
    u "(Je devrais parler à Penelope.)"

# game/v8/scene17.rpy:30
translate francais phn_penelope6_done_4f1997c1:

    # "*Knock knock knock*"
    "*TOC TOC TOC*"

# game/v8/scene17.rpy:34
translate francais phn_penelope6_done_477524e5:

    # u "Come in!"
    u "Entrez !"

# game/v8/scene17.rpy:38
translate francais phn_penelope6_done_6806a523:

    # ch "Hey man, you busy?"
    ch "Hey man, you busy?"

# game/v8/scene17.rpy:42
translate francais phn_penelope6_done_7709a633:

    # u "Naw, just studying."
    u "Nah, J'étudie c'est tout."

# game/v8/scene17.rpy:46
translate francais phn_penelope6_done_d3f66e85:

    # ch "Heh! Same, bro."
    ch "Eh ! Pareil pour moi, fréro."

# game/v8/scene17.rpy:50
translate francais phn_penelope6_done_0d8533cf:

    # ch "Listen, Ms. Rose is in trouble and needs our help. Some of us are heading over to help her move out of her house. You in?"
    ch "Écoute, Mme Rose a des problèmes et a besoin de notre aide. Certains d'entre nous vont l'aider à quitter sa maison. Tu viens ?"

# game/v8/scene17.rpy:53
translate francais phn_penelope6_done_52472f3f:

    # u "(Aww, I knew something was up.)"
    u "(Oh, je savais que quelque chose n'allait pas.)"

# game/v8/scene17.rpy:57
translate francais phn_penelope6_done_fb80a3c9:

    # u "Wow, what happened? Of course I'm in."
    u "Waouh, que s'est-il passé ? Bien sûr que je viens."

# game/v8/scene17.rpy:61
translate francais phn_penelope6_done_503de2c6:

    # ch "She's having some trouble with her old man. I don't know all the details..."
    ch "Elle a des problèmes avec son ex. Je ne connais pas tous les détails..."

# game/v8/scene17.rpy:62
translate francais phn_penelope6_done_267776dd:

    # ch "...but she sounded very upset when I ran into her a few minutes ago and I said we'd help."
    ch "... mais elle avait l'air très énervée quand je l'ai rencontrée il y a quelques minutes et j'ai dit que nous allions l'aider."

# game/v8/scene17.rpy:68
translate francais phn_penelope6_done_bfa2ca0c:

    # u "Let's go!"
    u "Allons-y!"

# game/v8/scene17.rpy:80
translate francais phn_penelope6_done_227600cb:

    # ro "Thank you so much, boys. I don't know how I'd get this all done before Mr. Rose gets home."
    ro "Merci beaucoup, les garçons. Je ne sais pas comment j'aurais fait tout ça avant que M. Rose ne rentre à la maison."

# game/v8/scene17.rpy:84
translate francais phn_penelope6_done_d454f4d2:

    # u "When will he be back?"
    u "Quand sera-t-il de retour ?"

# game/v8/scene17.rpy:90
translate francais phn_penelope6_done_4f1d9dac:

    # ro "I... I don't know."
    ro "Je... je ne sais pas."

# game/v8/scene17.rpy:94
translate francais phn_penelope6_done_f2d6cd60:

    # ch "Don't worry about it. We'll get you out of here."
    ch "Ne vous en faites pas. On va vous sortir d'ici."

# game/v8/scene17.rpy:98
translate francais phn_penelope6_done_5bd7b068:

    # u "We're all here to help. Just tell us what to do."
    u "Nous sommes tous là pour vous aider. Dites-nous simplement quoi faire."

# game/v8/scene17.rpy:102
translate francais phn_penelope6_done_7db5ebfa:

    # ro "Um... when you're done in here, there are more boxes in the kitchen."
    ro "Hum... quand tu as fini ici, il y a d'autres cartons dans la cuisine."

# game/v8/scene17.rpy:103
translate francais phn_penelope6_done_32463bdd:

    # ro "I brought what I could from the other rooms..."
    ro "J'ai apporté ce que j'ai pu des autres pièces..."

# game/v8/scene17.rpy:107
translate francais phn_penelope6_done_9a4b295e:

    # ro "...the ones I could carry."
    ro "... ceux que je pouvais porter."

# game/v8/scene17.rpy:111
translate francais phn_penelope6_done_313cc6e6:

    # u "Are you alright, Ms. Rose?"
    u "Vous allez bien, Mme Rose ?"

# game/v8/scene17.rpy:115
translate francais phn_penelope6_done_0b46e452:

    # ro "I'm alright, th-thank you. It... it's been a long time coming. I just want it to be over with."
    ro "Je vais bien, merci. Ça... ça fait longtemps que ça arrive. Je veux juste que ce soit fini.""

# game/v8/scene17.rpy:123
translate francais phn_penelope6_done_808acb7a:

    # ch "Did he hurt you?"
    ch "Est-ce qu'il vous fait du mal ?"

# game/v8/scene17.rpy:127
translate francais phn_penelope6_done_e03cccc3:

    # ro "He's had it rough. It's not him. It's..."
    ro "Il a eu sa violence. Ce n'est pas lui. C'est..."

# game/v8/scene17.rpy:131
translate francais phn_penelope6_done_f64ec35f:

    # ro "*Sniff* I don't want to be here anymore. I need to get out before he shows up."
    ro "*Sniff* Je ne veux plus être ici. Je dois sortir avant qu'il ne se montre."

# game/v8/scene17.rpy:135
translate francais phn_penelope6_done_6c879d0f:

    # u "Hey, we're here. Nothing's gonna happen to you."
    u "Hé, on est là. Rien ne va vous arriver."

# game/v8/scene17.rpy:139
translate francais phn_penelope6_done_d246e7ae:

    # ro "He's just not the same after the accident."
    ro "Il n'est plus le même depuis l'accident."

# game/v8/scene17.rpy:143
translate francais phn_penelope6_done_02f0913a:

    # u "(Ms. Rose is so upset, but we also really need to hurry if we're gonna finish this before her husband shows up.)"
    u "(Mme Rose est tellement bouleversée, mais nous devons aussi vraiment nous dépêcher si nous voulons en finir avant que son mari ne se présente.)"

# game/v8/scene17.rpy:150
translate francais phn_penelope6_done_fe59dd4e:

    # u "(This is important. There are enough Wolves to move boxes quickly.)"
    u "(Ceci est important. Il y a suffisamment de loups pour déplacer les boîtes rapidement.)"

# game/v8/scene17.rpy:154
translate francais phn_penelope6_done_c0905d9e:

    # ch "That's no excuse. You deserve to feel safe in your own home."
    ch "Ce n'est pas une excuse. Vous méritez de vous sentir en sécurité dans votre propre maison."

# game/v8/scene17.rpy:158
translate francais phn_penelope6_done_963b3625:

    # u "Yeah, and we're gonna make sure that happens."
    u "Ouais, et on va s'assurer que ça arrive."

# game/v8/scene17.rpy:162
translate francais phn_penelope6_done_e216c953:

    # ro "Thank you, boys. I know I'll be alright once I'm out of here."
    ro "Merci, les garçons. Je sais que j'irai bien une fois que je serai sorti d'ici."

# game/v8/scene17.rpy:166
translate francais phn_penelope6_done_af69c7be:

    # ro "I can't stand to look at these four walls anymore *Sniff*"
    ro "Je ne supporte plus de regarder ces quatre murs *Sniff*"

# game/v8/scene17.rpy:170
translate francais phn_penelope6_done_ef6af478:

    # ro "The whole place is...{w} tainted!"
    ro "Tout l'endroit est... {w} souillé !"

# game/v8/scene17.rpy:174
translate francais phn_penelope6_done_6aa8a1b9:

    # u "It won't be long Ms. Rose. We brought our best Wolves."
    u "Ce ne sera pas long, Mme Rose. Nous avons amené nos meilleurs loups."

# game/v8/scene17.rpy:178
translate francais phn_penelope6_done_df98a8b9:

    # ch "And it's the least we could do after everything you've done for us."
    ch "Et c'est le moins que nous puissions faire après tout ce que vous avez fait pour nous."

# game/v8/scene17.rpy:179
translate francais phn_penelope6_done_0e6cfb3b:

    # ch "You're our favorite teacher after all!"
    ch "Vous êtes notre professeur préféré après tout !"

# game/v8/scene17.rpy:183
translate francais phn_penelope6_done_0008d2d5:

    # ro "Really?"
    ro "Vraiment ?"

# game/v8/scene17.rpy:187
translate francais phn_penelope6_done_f75712e4:

    # u "Of course, Ms. Rose!"
    u "Bien sûr, Mme Rose !"

# game/v8/scene17.rpy:191
translate francais phn_penelope6_done_c5f20ef1:

    # imre "Mine too!"
    imre "Moi aussi !"

# game/v8/scene17.rpy:195
translate francais phn_penelope6_done_2d4696e7:

    # ro "Awww, this was from before the accident. Before everything... How did this happen?"
    ro "Oh, c'était avant l'accident. Avant tout... Comment est-ce arrivé ?"

# game/v8/scene17.rpy:201
translate francais phn_penelope6_done_be15eea4:

    # u "Don't worry. We'll be out of here in no time."
    u "Ne t'inquiète pas. Nous serons sortis d'ici en un rien de temps."

# game/v8/scene17.rpy:202
translate francais phn_penelope6_done_134c71ea:

    # u "I'll uh... go and get more boxes."
    u "Je vais euh... aller chercher d'autres boîtes."

# game/v8/scene17.rpy:206
translate francais phn_penelope6_done_3d22a601:

    # ro "Thank you *Sniff*"
    ro "Merci *Sniff*"

# game/v8/scene17.rpy:210
translate francais phn_penelope6_done_af69c7be_1:

    # ro "I can't stand to look at these four walls anymore *Sniff*"
    ro "Je ne supporte plus de regarder ces quatre murs *Sniff*"

# game/v8/scene17.rpy:214
translate francais phn_penelope6_done_98d58592:

    # imre "How bad is it?"
    imre "À quel point est-ce mauvais ?"

# game/v8/scene17.rpy:218
translate francais phn_penelope6_done_b7467919:

    # u "Bad. We gotta hurry."
    u "Mauvais. On doit se dépêcher."

# game/v8/scene17.rpy:222
translate francais phn_penelope6_done_0f9719ee:

    # imre "Here, I'll grab this one and you take that one."
    imre "Tenez, je vais prendre celui-ci et vous prenez celui-là."

# game/v8/scene17.rpy:230
translate francais phn_penelope6_done_0e6cfb3b_1:

    # ch "You're our favorite teacher after all!"
    ch "Vous êtes notre professeur préféré après tout !"

# game/v8/scene17.rpy:231
translate francais phn_penelope6_done_0008d2d5_1:

    # ro "Really?"
    ro "Vraiment ?"

# game/v8/scene17.rpy:235
translate francais phn_penelope6_done_6dd41966:

    # imre "Really!"
    imre "Vraiment !"

# game/v8/scene17.rpy:236
translate francais phn_penelope6_done_2d4696e7_1:

    # ro "Awww, this was from before the accident. Before everything... How did this happen?"
    ro "Oh, c'était avant l'accident. Avant tout... Comment est-ce arrivé ?"

# game/v8/scene17.rpy:241
translate francais phn_penelope6_done_39e0a03a:

    # u "Maybe talking about it will help. Get it all out."
    u "Peut-être qu'en parler aidera. Sortez tout. "

# game/v8/scene17.rpy:249
translate francais phn_penelope6_done_9f9155d0:

    # ro "Things were so good before the accident. But he..."
    ro "Les choses allaient si bien avant l'accident. Mais il..."

# game/v8/scene17.rpy:250
translate francais phn_penelope6_done_867ed379:

    # ro "...he's changed so much now. I told him to get rid of that damn motorcycle!"
    ro "... il a tellement changé maintenant. Je lui ai dit de se débarrasser de cette foutue moto !"

# game/v8/scene17.rpy:254
translate francais phn_penelope6_done_6db8db1e:

    # ro "I took care of him for weeks after the accident. I did everything for him."
    ro "Je me suis occupé de lui pendant des semaines après l'accident. J'ai tout fait pour lui."

# game/v8/scene17.rpy:258
translate francais phn_penelope6_done_bb0636c1:

    # ro "He broke both his legs and... They gave him some pain killers. At first I was giving them to him when he needed them..."
    ro "Il s'est cassé les deux jambes et... Ils lui ont donné des analgésiques. Au début je les lui donnais quand il en avait besoin..."

# game/v8/scene17.rpy:262
translate francais phn_penelope6_done_3f7d71a5:

    # ro "...but I had to go back to work and he started..."
    ro "...mais j'ai dû retourner au travail et il a commencé..."

# game/v8/scene17.rpy:266
translate francais phn_penelope6_done_c8414f0e:

    # ch "It's not your fault. He's an adult."
    ch "Ce n'est pas de ta faute. C'est un adulte."

# game/v8/scene17.rpy:270
translate francais phn_penelope6_done_ea2bb818:

    # u "And it's still not an excuse for..."
    u "Et ce n'est toujours pas une excuse pour..."

# game/v8/scene17.rpy:276
translate francais phn_penelope6_done_95bbef6d:

    # u "One moment."
    u "Un moment."

# game/v8/scene17.rpy:282
translate francais phn_penelope6_done_e2d5fe3e:

    # u "Hey Lauren, what's up?"
    u "Hé Lauren, quoi de neuf ?"

# game/v8/scene17.rpy:286
translate francais phn_penelope6_done_bb34d75a:

    # la "Hey [name], you wanna meet up? I've had enough Econ assignments for the day."
    la "Hé [name], tu veux me rencontrer ? J'ai eu assez de devoirs d'Eco pour la journée."

# game/v8/scene17.rpy:290
translate francais phn_penelope6_done_48b2624a:

    # u "Listen, I'm in the middle of something..."
    u "Écoute, je suis au milieu de quelque chose..."

# game/v8/scene17.rpy:295
translate francais phn_penelope6_done_67b2d8ad:

    # la "Whatcha doooin?"
    la "Que fais-tu ?"

# game/v8/scene17.rpy:299
translate francais phn_penelope6_done_777532dc:

    # u "Helping a... friend."
    u "Aider un... ami."

# game/v8/scene17.rpy:303
translate francais phn_penelope6_done_faa3f1ac:

    # la "OK... talk later I guess?"
    la "OK... On parlera plus tard je suppose ?"

# game/v8/scene17.rpy:307
translate francais phn_penelope6_done_953a9579:

    # u "Yeah, bye."
    u "Ouais au revoir."

# game/v8/scene17.rpy:312
translate francais phn_penelope6_done_dbb46555:

    # la "Oh, well let me know if you get time later."
    la "Oh, tiens moi au courant si tu as le temps plus tard."

# game/v8/scene17.rpy:316
translate francais phn_penelope6_done_7d4b720b:

    # u "Sure, bye."
    u "Bien sûr, au revoir."

# game/v8/scene17.rpy:322
translate francais phn_penelope6_done_01897609:

    # ro "...been so angry lately."
    ro "... été tellement en colère ces derniers temps."

# game/v8/scene17.rpy:326
translate francais phn_penelope6_done_e05f8d42:

    # ro "I thought he was doing better but last night..."
    ro "Je pensais qu'il allait mieux mais hier soir..."

# game/v8/scene17.rpy:330
translate francais phn_penelope6_done_6b26ed43:

    # ch "You sure you don't want us to call the police? Or someone? Do you have family nearby?"
    ch "Vous êtes sûre que vous ne voulez pas qu'on appelle la police ? Ou quelqu'un ? Avez-vous de la famille à proximité ?"

# game/v8/scene17.rpy:334
translate francais phn_penelope6_done_bc598a21:

    # ro "I'm sure I'll be fine when I get out of here."
    ro "Je suis sûre que j'irai bien quand je sortirai d'ici."

# game/v8/scene17.rpy:338
translate francais phn_penelope6_done_3c9398f3:

    # u "(Man... her situation is so bad.)"
    u "(Mec... sa situation est si mauvaise.)"

# game/v8/scene17.rpy:339
translate francais phn_penelope6_done_ea60a788:

    # u "You don't deserve this Ms. Rose."
    u "Vous ne méritez pas ça, Mme Rose."

# game/v8/scene17.rpy:343
translate francais phn_penelope6_done_bb528ecf:

    # ch "Let's pick up the pace, Wolves!"
    ch "Accélérez le rythme, les Wolves !"

# game/v8/scene17.rpy:347
translate francais phn_penelope6_done_67f8a638:

    # imre "Other room's done. Just finishing up here."
    imre "L'autre pièce est terminée. Je finis juste ici."

# game/v8/scene17.rpy:351
translate francais phn_penelope6_done_6d9dbbff:

    # u "Told ya we'd take care of you Ms. Rose!"
    u "Je vous avais dit qu'on prendrait soin de vous Mlle Rose !"

# game/v8/scene17.rpy:355
translate francais phn_penelope6_done_b3409e2d:

    # ro "I owe you boys some pizza after this !"
    ro "Je vous dois une pizza après ça !"

# game/v8/scene17.rpy:359
translate francais phn_penelope6_done_9e2bb44c:

    # ch "We're just glad to help, Ms. Rose."
    ch "We're just glad to help, Ms. Rose."

# game/v8/scene17.rpy:363
translate francais phn_penelope6_done_30f8792d:

    # imre "And this room is done!"
    imre "Et cette pièce est terminée !"

translate francais strings:

    # game/v8/scene17.rpy:11
    old "Hey, how you holding up?"
    new "Hé, comment vous tenez le coup ?"

    # game/v8/scene17.rpy:12
    old "Better, thanks to you"
    new "Mieux, grâce à vous"

    # game/v8/scene17.rpy:13
    old "No problem. I'm here for you. Let me know if you need anything else."
    new "Aucun problème. Je suis là pour vous. Faites moi savoir si vous avez besoin de quoique ce soit d'autre."

    # game/v8/scene17.rpy:14
    old "Thank you! :)"
    new "Merci ! :)"

    # game/v8/scene17.rpy:145
    old "Stay to console her"
    new "Reste pour la consoler"

    # game/v8/scene17.rpy:145
    old "Focus on moving the boxes"
    new "Concentre-toi sur le déplacement des boîtes"

