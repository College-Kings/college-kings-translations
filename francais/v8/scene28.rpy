# TODO: Translation updated at 2021-11-25 16:40

# game/v8/scene28.rpy:18
translate francais drug_deal_w_josh_7b255707:

    # jo "(quietly) Looks like we're a little early. Keep your eyes open and don't say anything, okay?"
    jo "*à voix basse* On dirait qu'on est un peu en avance. Garde les yeux ouverts et ne dis rien, ok ?"

# game/v8/scene28.rpy:23
translate francais drug_deal_w_josh_a93f4880:

    # u "(quietly) Fuck, it stinks here."
    u "*à voix basse* Putain, ça pue ici."

# game/v8/scene28.rpy:28
translate francais drug_deal_w_josh_b06969e2:

    # jo "(quietly) Yeah. You ready for this?"
    jo "*à voix basse* Ouais. T'es prêt ?"

# game/v8/scene28.rpy:33
translate francais drug_deal_w_josh_f3a9b291:

    # u "(quietly) Yeah, okay. What do I do if..."
    u "*à voix basse* Ouais, ok. Je fais quoi si..."

# game/v8/scene28.rpy:43
translate francais drug_deal_w_josh_cfb156a2:

    # je "'Sup."
    je "Yo."

# game/v8/scene28.rpy:48
translate francais drug_deal_w_josh_96e7fb41:

    # jo "'Sup."
    jo "Yo."

# game/v8/scene28.rpy:53
translate francais drug_deal_w_josh_5def5c30:

    # je "You got the shit?"
    je "T'as la came ?"

# game/v8/scene28.rpy:58
translate francais drug_deal_w_josh_a96dcb3e:

    # jo "Yeah. You got the cash?"
    jo "Ouais. T'as l'argent ?"

# game/v8/scene28.rpy:63
translate francais drug_deal_w_josh_83f14cd4:

    # je "Yeah. Let me see the stuff."
    je "Ouais. fais voir."

# game/v8/scene28.rpy:68
translate francais drug_deal_w_josh_fcebfc33:

    # jo "Let me see the cash."
    jo "Fais voir l'argent d'abord !"

# game/v8/scene28.rpy:80
translate francais drug_deal_w_josh_f68777bb:

    # je "See, this is where you're under the wrong impression you're in control of this situation. My buddy here, Lars, has a quick temper and fists like rock. Pretty sure I don't gotta spell shit out for ya, do I?"
    je "D'où t'as cru avoir l'impression de contrôler le deal ? Mon pote Lars, là. C'est un  rapide avec des poings de pierre. Je suis sûr que j'ai pas besoin d'expliquer la merde dans laquelle vous êtes, hein ?"

# game/v8/scene28.rpy:85
translate francais drug_deal_w_josh_d64a601f:

    # jo "Spell out what? You've got the cash, I've got the shit, what's the problem?"
    jo "Expliquer quoi ? T'as l'argent, j'ai la came, où est le problème ?"

# game/v8/scene28.rpy:90
translate francais drug_deal_w_josh_dacd3592:

    # je "Well gee, looks like you aren't too bright. Lars, enlighten the man, will ya?"
    je "On dirait que t'es pas très intelligent. Lars, va faire comprendre à ce mec ce qui va se passer."

# game/v8/scene28.rpy:100
translate francais drug_deal_w_josh_468329a6:

    # je "If I were you, I'd just hand over the shit now before you end up with a nice, shiny dent in your skull. Save Lars the effort. You dig?"
    je "Si j'étais toi, j'donnerais la came maintenant avant qu'tu finisses avec une belle et brillante barre dans ton crâne. En plus, ça évitera à Lars de faire des efforts. Il aime pas trop ça, t'as compris ?"

# game/v8/scene28.rpy:118
translate francais int_deal_w_josh_3ae5da6b:

    # u "Hey back off."
    u "Hey, recule."

# game/v8/scene28.rpy:123
translate francais int_deal_w_josh_c78d3d3d:

    # je "Well, lookie here, a real hero. Lars, show him what happens to heroes."
    je "Eh bien, regardez moi ça, un vrai héros. Lars, montre-lui c'qui arrive aux super-héros."

# game/v8/scene28.rpy:491
translate francais beat_lars_369b2594:

    # u "(Oh shit)"
    u "(Oh merde)"

# game/v8/scene28.rpy:511
translate francais no_int_deal_w_josh_f097ed12:

    # jo "No. Fuck you! I'm not giving you shit! You want it, you pay me."
    jo "Non. Va te faire foutre ! Je te donne pas la came ! Tu la veux, tu m'payes."

# game/v8/scene28.rpy:516
translate francais no_int_deal_w_josh_41dd6e28:

    # je "No, no, no...fuck YOU."
    je "Non, non, non... \"Va te faire FOUTRE\" ???"

# game/v8/scene28.rpy:531
translate francais no_int_deal_w_josh_9651f818:

    # je "You call the cops, I'll find you and pull your tongue down and tie it to your dick, got it?"
    je "Si t'appelles les flics, j'te jure que je te retrouverai, je t'arracherai la langue et l'attacherai à ta queue, TU COMPRENDS ?"

# game/v8/scene28.rpy:559
translate francais dodged_pipe_07b0b527:

    # u "Fuck you!"
    u "Nique toi !"

# game/v8/scene28.rpy:569
translate francais dodged_pipe_e5cc030a:

    # u "Fucking piece of shit!"
    u "Putain de tas de merde !"

# game/v8/scene28.rpy:598
translate francais hit_with_pipe_fdf16ca3:

    # je "Hahaha, see ya, 'hero'."
    je "Hahaha, à plus tard, \"héros\"."

# game/v8/scene28.rpy:631
translate francais beat_by_lars_53d5aab9:

    # je "A valiant effort, hero, but not good enough. Let's go Lars."
    je "Un effort courageux, héros, mais pas assez bon. On s'casse, Lars."

# game/v8/scene28.rpy:653
translate francais volley_joe_9ca302d5:

    # u "Cocksucker..."
    u "Fils de pute..."

# game/v8/scene28.rpy:666
translate francais check_on_josh_72dfa49e:

    # u "Hey Josh, you alright? Can you move?"
    u "Hey Josh, tu vas bien ? Tu peux bouger ?"

# game/v8/scene28.rpy:671
translate francais check_on_josh_661b0966:

    # jo "Fuck man, this wasn't supposed to happen..."
    jo "Putain, mec, c'était pas censé se passer comme ça..."

# game/v8/scene28.rpy:676
translate francais check_on_josh_fad7082b:

    # u "We don't have time for this. We gotta go. C'mon, let's get you home."
    u "On n'a pas le temps pour ça. On doit y aller. Allez, je te ramène à la maison."

translate francais strings:

    # game/v8/scene28.rpy:102
    old "Intervene"
    new "Intervenir"

    # game/v8/scene28.rpy:102
    old "Don't intervene"
    new "Ne pas intervenir"

    # game/v8/scene28.rpy:499
    old "Duck"
    new "Esquiver"

    # game/v8/scene28.rpy:579
    old "Kick Joe"
    new "Shooter Joe"

# TODO: Translation updated at 2021-11-19 17:55

# game/v8/scene28.rpy:18
translate francais drug_deal_w_josh_abd9e49e:

    # jo "(Quietly) Looks like we're a little early. Keep your eyes open and don't say anything, okay?"
    jo "*à voix basse* On dirait qu'on est un peu en avance. Garde les yeux ouverts et ne dis rien, ok ?"

# game/v8/scene28.rpy:23
translate francais drug_deal_w_josh_8df4069b:

    # u "(Quietly) Fuck, it stinks here."
    u "*à voix basse* Putain, ça pue ici."

# game/v8/scene28.rpy:28
translate francais drug_deal_w_josh_7f9bcfba:

    # jo "(Quietly) Yeah. You ready for this?"
    jo "*à voix basse* Ouais. T'es prêt ?"

# game/v8/scene28.rpy:33
translate francais drug_deal_w_josh_11962f29:

    # u "(Quietly) Yeah, okay. What do I do if..."
    u "*à voix basse*  Ouais, ok. Je fais quoi si..."

# game/v8/scene28.rpy:118
translate francais int_deal_w_josh_663956cb:

    # u "Hey, back off."
    u "Hey, recule."

# game/v8/scene28.rpy:510
translate francais no_int_deal_w_josh_79dde407:

    # je "No, no, no... fuck YOU."
    je "Non, non, non... TOI Va te faire FOUTRE !"

# game/v8/scene28.rpy:592
translate francais hit_with_pipe_64683e6f:

    # je "Hahaha, see ya, \"hero\"."
    je "Hahaha, à plus tard, \"héros\"."

# game/v8/scene28.rpy:625
translate francais beat_by_lars_51bbb9fc:

    # je "A valiant effort, hero, but not good enough. Let's go, Lars."
    je "Un effort courageux, héros, mais pas assez bon. On s'casse, Lars."
