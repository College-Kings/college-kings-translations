# TODO: Translation updated at 2021-11-26 17:26

# game/v8/scene38.rpy:23
translate francais seb_in_room_902c3bc0:

    # u "(Wonder who that is.)"
    u "(Je me demande qui c'est.)"

# game/v8/scene38.rpy:31
translate francais seb_in_room_1d52d167:

    # se "Hey, bro! What's shaking? You got a minute?"
    se "Hé, mon frère ! T'as une minute ?"

# game/v8/scene38.rpy:36
translate francais seb_in_room_eb119fd2:

    # u "Yeah, of course, come on in."
    u "Oui, bien sûr, entre."

# game/v8/scene38.rpy:46
translate francais seb_in_room_50b58ffa:

    # se "How you settling in? You cool with being here?"
    se "Comment tu te sens ? Ça te plaît d'être ici ?"

# game/v8/scene38.rpy:51
translate francais seb_in_room_9a467561:

    # u "Yeah! It's great. Can't think of anything I'm missing."
    u "Ouais ! C'est génial. Je ne vois pas ce qui me manque."

# game/v8/scene38.rpy:56
translate francais seb_in_room_18e518c9:

    # se "Except some deco on the walls eh? Hahaha!"
    se "A part quelques décorations sur les murs, hein ? Hahaha !"

# game/v8/scene38.rpy:61
translate francais seb_in_room_cc717b25:

    # u "Ahh, yeah maybe. I haven't really had time to add a personal touch. I'll get there eventually."
    u "Ahh, oui peut-être. Je n'ai pas vraiment eu le temps d'ajouter une touche personnelle. Quand j'aurai un peu de temps, j'y penserai."

# game/v8/scene38.rpy:66
translate francais seb_in_room_ab312397:

    # se "Ah yeah, I hear that. Took me almost a month to finally get my cave looking sexy and shit haha."
    se "Ah ouais, je comprends. Ça m'a pris presque un mois pour que ma grotte soit enfin sexy et tout haha."

# game/v8/scene38.rpy:71
translate francais seb_in_room_bb0eac50:

    # u "I'm not that great at decorating. Maybe you could give me some pointers some time?"
    u "Je ne suis pas très doué pour la décoration. Peut-être que tu pourrais me donner quelques conseils un jour ?"

# game/v8/scene38.rpy:76
translate francais seb_in_room_940cf87d:

    # se "I'd be happy to, bro. Which reminds me, I came here to ask you something."
    se "J'en serais heureux, mon frère. Ce qui me rappelle, je suis venu ici pour te demander quelque chose."

# game/v8/scene38.rpy:86
translate francais seb_in_room_8d5eacd9:

    # u "Yeah shoot. What's up?"
    u "Ouais, vas-y. Qu'est-ce qu'il y a ?"

# game/v8/scene38.rpy:91
translate francais seb_in_room_37538ebe:

    # se "Nothing major, man. Just wanted to ask a small favor."
    se "Rien d'important, mec. Je voulais juste te demander une petite faveur."

# game/v8/scene38.rpy:96
translate francais seb_in_room_f843bcaa:

    # u "If I can, I will. What's going on man?"
    u "Si je peux, je le ferai. Qu'est-ce qui se passe ?"

# game/v8/scene38.rpy:101
translate francais seb_in_room_db23c237:

    # se "Well, me and Marcus usually do this together, but he's unavailable at the moment. Something came up and he had to split. Something about his father. I didn't press."
    se "Marcus et moi faisons habituellement ça ensemble, mais il n'est pas disponible pour le moment. Quelque chose est arrivé et il a dû partir. Quelque chose à propos de son père. Je n'ai pas insisté."

# game/v8/scene38.rpy:108
translate francais seb_in_room_8acbb78a:

    # u "You have a hard time asking a favor, don't you?"
    u "Tu as du mal à demander une faveur, n'est-ce pas ?"

# game/v8/scene38.rpy:113
translate francais seb_in_room_e6e97d1c:

    # se "Fuck man, do I! Haha. I've always just been a \"Do It Yourself\" kind of guy, even for stupid and small things."
    se "Putain, c'est vrai ! Haha. J'ai toujours été un gars du genre \"fais-le toi-même\", même pour les petites choses stupides."

# game/v8/scene38.rpy:118
translate francais seb_in_room_66a7a21c:

    # u "Like I said, Wolves stick together, you need a hand, I'll do whatever I can."
    u "Comme je l'ai dit, les Wolves se serrent les coudes, si tu as besoin d'un coup de main, je ferai tout ce que je peux."

# game/v8/scene38.rpy:123
translate francais seb_in_room_bfd96c00:

    # se "Thanks [name]. Ok well, like I was saying, Marcus usually does this but he can't tonight. I was wondering if you want to climb the hospital with me?"
    se "Merci [name]. Ok, comme je le disais, Marcus le fait d'habitude mais il ne peut pas ce soir. Je me demandais si tu voulais escalader l'hôpital avec moi ?"

# game/v8/scene38.rpy:128
translate francais seb_in_room_4cc24213:

    # u "Umm, hospital? Climb? I'm not following."
    u "Humm, l'hôpital ? Escalade ? Je ne comprends pas vraiment."

# game/v8/scene38.rpy:133
translate francais seb_in_room_a39c31ea:

    # se "Haha, I'm not surprised. Me and Marcus climb the hospital, from the ground to roof, -Sebastian points down then up- no equipment, just pure balls and stupidity haha. It's also a great workout bro. Really gets those muscles working and that blood pumping. You in?"
    se "Haha, je ne suis pas surpris. Moi et Marcus, on escalade l'hôpital, du sol au toit, -Sebastian pointe vers le bas puis vers le haut- sans équipement, juste des couilles et de la stupidité pure haha. C'est aussi une bonne séance d'entraînement mec. Ça fait vraiment travailler les muscles et pomper le sang. Tu en es ?"

# game/v8/scene38.rpy:138
translate francais seb_in_room_3000c55e:

    # u "Damn, that sounds dangerous. What if we fall?"
    u "Putain, ça a l'air dangereux. Et si on tombe ?"

# game/v8/scene38.rpy:143
translate francais seb_in_room_4ba808c1:

    # se "Trust me [name], we have the perfect route up. If you want, you can always use a safety hook. I'll show you how to use it."
    se "Fais-moi confiance [name], on a l'itinéraire parfait. Si tu veux, tu peux toujours utiliser un crochet de sécurité. Je vais te montrer comment l'utiliser."

# game/v8/scene38.rpy:148
translate francais seb_in_room_1cca1232:

    # u "Yeah sure, fuck it. Why not?"
    u "Oui, bien sûr, merde. Pourquoi pas ?"

# game/v8/scene38.rpy:153
translate francais seb_in_room_b918778c:

    # se "WOOO! Awesome! Thanks, buddy! I knew I could count on you!"
    se "WOOO ! Génial ! Merci, mon pote ! Je savais que je pouvais compter sur toi !"

# game/v8/scene38.rpy:158
translate francais seb_in_room_dc014268:

    # u "Hahaha! No problem, man."
    u "Hahaha ! Pas de problème, mec."

# game/v8/scene38.rpy:163
translate francais seb_in_room_36679b6b:

    # se "Alright well, we gotta get moving. We'll go together. It's a nice easy jog to warm up."
    se "Très bien, on doit y aller. On ira ensemble. On fera un jogging facile pour s'échauffer."

# game/v8/scene38.rpy:168
translate francais seb_in_room_cb15c818:

    # u "Way ahead of you, let's rock."
    u "J'ai une longueur d'avance sur toi, c'est parti."

# game/v8/scene38.rpy:173
translate francais seb_in_room_abd362e9:

    # se "This is gonna be so fuckin great, you'll see!"
    se "Ça va être vraiment génial, tu vas voir !"

# TODO: Translation updated at 2021-11-19 17:55

# game/v8/scene38.rpy:130
translate francais seb_in_room_3f3a6bc0:

    # se "Haha, I'm not surprised. Me and Marcus climb the hospital, from the ground to roof, no equipment, just pure balls and stupidity haha."
    se "Haha, je ne suis pas surpris. Marcus et moi avons escaladé l'hôpital, du sol au toit, sans équipement, juste des couilles pures et de la stupidité haha."

# game/v8/scene38.rpy:131
translate francais seb_in_room_437fec49:

    # se "It's also a great workout bro. Really gets those muscles working and that blood pumping. You in?"
    se "C'est aussi un excellent entraînement. Ça fait vraiment travailler les muscles et circuler le sang. Tu en es ?"
