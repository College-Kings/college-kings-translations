# TODO: Translation updated at 2021-11-26 16:51

# game/v8/scene37.rpy:12
translate francais v8_tues_evening_565c0af2:

    # u "(I must be imagining things... Or maybe I should just text someone?)"
    u "(Je dois être en train d'imaginer des choses... Ou peut-être que je devrais juste envoyer un texto à quelqu'un ?)"

# game/v8/scene37.rpy:40
translate francais v8s37_phoneContinue1_6b32f074:

    # u "I should reply to Lauren."
    u "Je devrais répondre à Lauren."

# game/v8/scene37.rpy:64
translate francais v8s37_phoneContinue2_b2a97980:

    # u "I should reply to Riley."
    u "Je devrais répondre à Riley."

# game/v8/scene37.rpy:67
translate francais v8s37_phoneContinue2_0d29d3a6:

    # u "(I guess this is just not my day for studying. Great job, [name]!)"
    u "(Je crois que ce n'est pas mon jour pour étudier. Bon travail, [name] !)"

# game/v8/scene37.rpy:72
translate francais v8s37_phoneContinue2_03be943f:

    # u "(If only something interesting would happen...)"
    u "(Si seulement quelque chose d'intéressant pouvait arriver...)"

# game/v8/scene37.rpy:80
translate francais v8s37_phoneContinue2_565c0af2:

    # u "(I must be imagining things... Or maybe I should just text someone?)"
    u "(Je dois être en train d'imaginer des choses... Ou peut-être que je devrais juste envoyer un texto à quelqu'un ?)"

# game/v8/scene37.rpy:108
translate francais v8s37_phoneContinue3_6b32f074:

    # u "I should reply to Lauren."
    u "Je devrais répondre à Lauren."

# game/v8/scene37.rpy:132
translate francais v8s37_phoneContinue4_95d895aa:

    # u "I should reply to my phone."
    u "Je devrais répondre à mon téléphone."

# game/v8/scene37.rpy:135
translate francais v8s37_phoneContinue4_0d29d3a6:

    # u "(I guess this is just not my day for studying. Great job, [name]!)"
    u "(Je crois que ce n'est pas mon jour pour étudier. Bon travail, [name] !)"

# game/v8/scene37.rpy:140
translate francais v8s37_phoneContinue4_03be943f:

    # u "(If only something interesting would happen...)"
    u "(Si seulement quelque chose d'intéressant pouvait arriver...)"

translate francais strings:

    # game/v8/scene37.rpy:18
    old "Still fine with drinking instead of making out with me?"
    new "Ça ne te dérange toujours pas de boire au lieu de sortir avec moi ?"

    # game/v8/scene37.rpy:19
    old "Of course, that drink was just my cup of tea. :)"
    new "Bien sûr, cette boisson était juste ma tasse de thé. :)"

    # game/v8/scene37.rpy:20
    old "Oh haha, is that so? I think it at least poured a bit of courage in there."
    new "Oh haha, c'est vrai ? Il me semble que tu as versé un peu de courage dedans."

    # game/v8/scene37.rpy:21
    old "Don't flatter yourself, witty cowboy. :)"
    new "Ne te flatte pas, cow-boy plein d'esprit. :)"

    # game/v8/scene37.rpy:22
    old "You flatter me, ma'am."
    new "C'est vous qui me flattez, m'dame."

    # game/v8/scene37.rpy:23
    old "Miss you."
    new "Tu me manques."

    # game/v8/scene37.rpy:24
    old "Kiss you*:)"
    new "Je t'embrasse* :)"

    # game/v8/scene37.rpy:28
    old "Hey there, could you help me with studying?"
    new "Salut, tu pourrais m'aider à étudier ?"

    # game/v8/scene37.rpy:29
    old "Umm, sure?"
    new "Umm, sûr ?"

    # game/v8/scene37.rpy:30
    old "I'm puzzled just as much as I was back on that bench..."
    new "Je suis tout aussi perplexe que je l'étais sur ce banc..."

    # game/v8/scene37.rpy:31
    old "You texted me just to make that reference, right?"
    new "Tu m'as envoyé un texto juste pour faire cette référence, c'est ça ?"

    # game/v8/scene37.rpy:32
    old "Maybe?"
    new "Peut-être ?"

    # game/v8/scene37.rpy:33
    old "Seriously [name], I have things to do!"
    new "Sérieusement [name], j'ai des choses à faire !"

    # game/v8/scene37.rpy:34
    old "Later then."
    new "À plus tard alors."

    # game/v8/scene37.rpy:44
    old "If your legs were lies, you could call me a liar. For I would be gladly spreading them."
    new "Si tes jambes étaient des mensonges, tu pourrais me traiter de menteur. Car je serais heureux de les étendre."

    # game/v8/scene37.rpy:45
    old "That was the most corny, idiotic joke I've ever heard, Mr. Liar. :)"
    new "C'est la blague la plus ringarde et la plus idiote que j'aie jamais entendue, Mr Menteur. :)"

    # game/v8/scene37.rpy:46
    old "Thinking of you makes it harder to study. And in some other places as well..."
    new "Penser à toi rend les études plus Dures. Et à un autre endroit aussi..."

    # game/v8/scene37.rpy:47
    old "I'll be the judge of that. :)"
    new "C'est à moi d'en juger. :)"

    # game/v8/scene37.rpy:48
    old "Can't wait. :)"
    new "J'ai hâte. :)"

    # game/v8/scene37.rpy:51
    old "I have a serious question to ask you."
    new "J'ai une question sérieuse à te poser."

    # game/v8/scene37.rpy:52
    old "What is it?"
    new "Qu'est-ce que c'est ?"

    # game/v8/scene37.rpy:53
    old "You get to chose between options A and B."
    new "Tu peux choisir entre les options A et B."

    # game/v8/scene37.rpy:54
    old "OK?"
    new "Vas-y !"

    # game/v8/scene37.rpy:55
    old "Option A is to make out with Mr. Lee."
    new "L'option A est de sortir avec Mr Lee."

    # game/v8/scene37.rpy:56
    old "B. I choose B. Option B."
    new "B. Je choisis B. Option B."

    # game/v8/scene37.rpy:57
    old "Haha"
    new "Haha"

    # game/v8/scene37.rpy:58
    old "*yuck*"
    new "*beurk*"
