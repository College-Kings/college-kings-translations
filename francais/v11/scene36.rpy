# TODO: Translation updated at 2021-12-12 15:42

# game/v11/scene36.rpy:20
translate francais v11_imre_ryan_grapple_fbe20b7e:

    # lee "Have you two lost your minds? Behave yourselves!"
    lee "Vous avez perdu la tête tous les deux ? Comportez-vous bien !"

# game/v11/scene36.rpy:46
translate francais v11_imre_ryan_grapple_66874e1b:

    # lee "[name], are you really going to just stand there?"
    lee "[name], allez-vous vraiment rester planté là ?"

# game/v11/scene36.rpy:51
translate francais v11_imre_ryan_grapple_120a53eb:

    # u "Sorry, I-"
    u "Désolé, je..."

# game/v11/scene36.rpy:61
translate francais v11_imre_ryan_grapple_308d2a8f:

    # lee "That is enough! Are you two children?! Never would I have thought I'd be separating school yard fights on a college trip."
    lee "Ça suffit ! Êtes-vous deux gamins ?! Je n'aurais jamais pensé que je séparerai des bagarres de cour d'école pendant un voyage universitaire."

# game/v11/scene36.rpy:66
translate francais v11_imre_ryan_grapple_702fb1d8:

    # ry "You didn't hear what this smartass said to me!"
    ry "Vous n'avez pas entendu ce que ce petit malin m'a dit !"

# game/v11/scene36.rpy:71
translate francais v11_imre_ryan_grapple_5a35e30c:

    # imre "I meant what I said, bro."
    imre "J'pensais ce que j'ai dit, frère."

# game/v11/scene36.rpy:90
translate francais v11_imre_ryan_grapple_17c26900:

    # lee "DID YOU NOT HEAR ME?!"
    lee "VOUS NE M'AVEZ PAS ENTENDU ?!"

# game/v11/scene36.rpy:102
translate francais v11_imre_ryan_grapple_3f1294c4:

    # lee "Clearly you are both deaf or incompetent. I can't even enjoy my breakfast and a morning coffee? Imre, what did you say to him?"
    lee "Il est clair que vous êtes à la fois sourd et irresponsable. Je ne peux même pas apprécier mon petit déjeuner et un café matinal ? Imre, qu'est-ce que vous lui avez dit ?"

# game/v11/scene36.rpy:107
translate francais v11_imre_ryan_grapple_bf8224d4:

    # imre "I said he was scrawny."
    imre "J'ai dit qu'il était maigre."

# game/v11/scene36.rpy:112
translate francais v11_imre_ryan_grapple_bd22945b:

    # lee "What provoked you to say that?"
    lee "Qu'est-ce qui vous a poussé à dire ça ?"

# game/v11/scene36.rpy:117
translate francais v11_imre_ryan_grapple_276f6202:

    # imre "Facts. *Chuckles*"
    imre "Les faits. *Rire*"

# game/v11/scene36.rpy:122
translate francais v11_imre_ryan_grapple_41532f24:

    # ry "You said it to be an ass. And it's not even that, it's the way he said it. He walked past me and said \"Hey little scrawny bitch\" out of nowhere, for no reason."
    ry "Tu l'as dit pour être un connard. Et ce n'est même pas ça, c'est la façon dont il l'a dit. Il est passé devant moi et a dit : \"Hé, la petite pute maigrichonne\", sans raison."

# game/v11/scene36.rpy:124
translate francais v11_imre_ryan_grapple_2e4d9141:

    # ry "I told him to leave me alone and that I didn't have time for his bullshit, then he said it again by mocking me so I knocked the shit out of him."
    ry "Je lui ai dit de me lâcher et que je n'avais pas le temps pour ses conneries, puis il a recommencé en se moquant de moi alors je lui ai mis une raclée."

# game/v11/scene36.rpy:129
translate francais v11_imre_ryan_grapple_62652cd2:

    # imre "You say it like you actually landed a hit."
    imre "Tu l'dis comme si t'avais réussi à toucher le gros lot."

# game/v11/scene36.rpy:134
translate francais v11_imre_ryan_grapple_417a23cd:

    # lee "Is this some fraternity instigated altercation?"
    lee "Est-ce une altercation provoquée par les fraternités ?"

# game/v11/scene36.rpy:144
translate francais v11_imre_ryan_grapple_30f20e16:

    # lee "I believe I heard you say something about Imre smelling like a wet dog when we were at the airport, am I wrong or am I right, Ryan?"
    lee "Je crois vous avoir entendu dire qu'Imre sentait le chien mouillé quand nous étions à l'aéroport, je me trompe ou j'ai raison, Ryan ?"

# game/v11/scene36.rpy:149
translate francais v11_imre_ryan_grapple_99b993cc:

    # ry "Yeah, but it was true so I don't see the problem."
    ry "Oui, mais ça c'est vrai donc je ne vois pas le problème."

# game/v11/scene36.rpy:154
translate francais v11_imre_ryan_grapple_c83e487a:

    # u "Look, Mr. Lee. Can we just chalk this up to boys being boys?"
    u "Ecoutez, Mr Lee. Pouvons-nous simplement mettre ça sur le compte des garçons qui agissent avec leurs hormones ?"

# game/v11/scene36.rpy:159
translate francais v11_imre_ryan_grapple_b67c4ffc:

    # lee "You're right, boys will be boys, but men such as the three of you should not be acting like boys."
    lee "Vous avez raison, les garçons resteront des garçons, mais des hommes comme vous trois ne devraient pas agir comme des gamins."

# game/v11/scene36.rpy:164
translate francais v11_imre_ryan_grapple_efb11f30:

    # u "(\"Three of you...\", what did I do?)"
    u "(\"Comme vous trois...\", qu'est-ce que j'ai fait ?)"

# game/v11/scene36.rpy:169
translate francais v11_imre_ryan_grapple_9be7ae82:

    # lee "Since you all want to act like children, I'll treat you like children. You three are coming with me this morning. I'll be waiting for you in the car, please don't be long."
    lee "Puisque vous voulez tous agir comme des gamins, je vais vous traiter comme tels. Vous trois allez venir avec moi ce matin. Je vous attendrai dans la voiture, ne soyez pas trop long."

# game/v11/scene36.rpy:179
translate francais v11_imre_ryan_grapple_809141c8:

    # u "Look what you guys dragged me into."
    u "Regardez dans quoi vous m'avez entraîné."

# game/v11/scene36.rpy:184
translate francais v11_imre_ryan_grapple_e991c805:

    # imre "You got dragged into this when you decided to start being friends with that asshole."
    imre "T'as été impliqué dans tout ça quand t'as décidé d'être ami avec ce connard."

# game/v11/scene36.rpy:189
translate francais v11_imre_ryan_grapple_f19b4970:

    # ry "*Chuckles* I could say the same about you."
    ry "*Rire* Je pourrais dire la même chose de toi."

# game/v11/scene36.rpy:194
translate francais v11_imre_ryan_grapple_7e24586b:

    # u "Outside of seeing Mr. Lee get angry like that, there's nothing funny about this. I literally got woken up to the sound of you guys yelling and now I'm being dragged to who knows where."
    u "En dehors de voir Mr Lee s'énerver comme ça, il n'y a rien de drôle dans tout ça. Je me suis littéralement réveillé au son de votre engueulade et maintenant je suis traîné vers je ne sais où."

# game/v11/scene36.rpy:199
translate francais v11_imre_ryan_grapple_49e9e53e:

    # imre "Sorry dude."
    imre "Désolé, mec."

# game/v11/scene36.rpy:204
translate francais v11_imre_ryan_grapple_ad71b731:

    # ry "Sorry man."
    ry "Pardon mec."

# game/v11/scene36.rpy:209
translate francais v11_imre_ryan_grapple_4f229baa:

    # u "*Sighs* Don't worry about it. Maybe if we take too long he'll let it go. *Chuckles*"
    u "*Soupirs* Ne vous inquiétez pas pour ça. Peut-être que si nous prenons trop de temps, il laissera tomber. *Rire*"

# game/v11/scene36.rpy:214
translate francais v11_imre_ryan_grapple_0c2d99e3:

    # imre "That'd be nice, I'd rather go eat."
    imre "Ce s'rait cool, j'préférerais aller manger."

# game/v11/scene36.rpy:219
translate francais v11_imre_ryan_grapple_7baa47d9:

    # lee "I'M PRETTY SURE I SAID DON'T BE LONG!"
    lee "JE SUIS PRESQUE SÛR D'AVOIR DIT \"NE SOYEZ PAS TROP LONG\" !"

# game/v11/scene36.rpy:224
translate francais v11_imre_ryan_grapple_2fc38ecf:

    # u "Never mind."
    u "Bon, tant pis."

# game/v11/scene36.rpy:239
translate francais v11_imre_ryan_grapple_11c3d5ac:

    # lee "I want a quiet car ride, so try and get yourselves in a relaxing mood. Let's go."
    lee "Je veux un trajet tranquille en voiture, alors essayez de vous mettre dans une ambiance relaxante. Allons-y."
