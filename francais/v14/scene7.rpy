# TODO: Translation updated at 2022-03-17 15:15

# game/v14/scene7.rpy:22
translate francais v14s07_5d5641c1:

    # ro "Students, thank you all for being quick and ready to head back home."
    ro "Les jeunes, merci à tous d'être rapides et prêts à rentrer chez vous."

# game/v14/scene7.rpy:27
translate francais v14s07_cd2480af:

    # ro "I've enjoyed this trip quite a bit, but I am very much ready to head home."
    ro "J'ai vraiment aimé ce voyage, mais je suis fin prête à rentrer à la maison."

# game/v14/scene7.rpy:32
translate francais v14s07_764e865c:

    # ch "I couldn't agree more."
    ch "Je suis tout à fait d'accord."

# game/v14/scene7.rpy:37
translate francais v14s07_fb4f432f:

    # ro "We all have a life to get back to in San Vallejo and the lives you have on campus, certainly need you all back."
    ro "Nous avons tous une vie qui nous attend à San Vallejo et vous avez besoin de vous retrouver sur le campus."

# game/v14/scene7.rpy:42
translate francais v14s07_3d50375e:

    # ro "I'm aware that you all became very involved in this adventure..."
    ro "Je suis consciente que vous étiez tous très impliqués dans cette aventure..."

# game/v14/scene7.rpy:47
translate francais v14s07_4a77a35b:

    # au "*Whispers* Involved with each other..."
    au "*Chuchote* Impliqués les uns avec les autres..."

# game/v14/scene7.rpy:57
translate francais v14s07_81e42504:

    # ri "*Whispers* Oh my god, Aubrey! Haha, shhh..."
    ri "*Chuchote* Oh là là, Aubrey ! Haha, chuuut..."

# game/v14/scene7.rpy:62
translate francais v14s07_1f5f7273:

    # u "(Look at them. All... lovey dovey.)"
    u "(Regarde-les. Elles sont... très amoureuses.)"

# game/v14/scene7.rpy:67
translate francais v14s07_b85bdb1d:

    # ro "There were memories made..."
    ro "Des souvenirs ont été créés..."

# game/v14/scene7.rpy:73
translate francais v14s07_752d5669:

    # ro "Some of my most unforgettable experiences happened on this journey, as have yours."
    ro "Certaines de mes expériences les plus mémorables ont eu lieu pendant ce voyage, tout comme pour vous, j'imagine."

# game/v14/scene7.rpy:78
translate francais v14s07_61f57433:

    # u "(Okay Rose, let's try to be a little less obvious here.)"
    u "(Ok Rose, essaie d'être un peu moins transparente, là)."

# game/v14/scene7.rpy:83
translate francais v14s07_fddd5eec:

    # ro "Maybe one day I'll come back here to create even more memories, but with someone special."
    ro "Peut-être qu'un jour, je reviendrai ici pour y créer davantage de souvenirs, mais... avec quelqu'un de spécial."

# game/v14/scene7.rpy:88
translate francais v14s07_1d9e426a:

    # ro "You should all consider coming back to these beautiful countries some time, and enjoy it with your new found freedom and someone special by your side."
    ro "Vous devriez tous un jour envisager de revenir dans ces beaux pays, et afin de pouvoir en profiter avec votre liberté retrouvée et quelqu'un de spécial à vos côtés."

# game/v14/scene7.rpy:93
translate francais v14s07_87c36052:

    # imre "*Laughs* Not anytime soon."
    imre "*Rire* Pas avant longtemps."

# game/v14/scene7.rpy:98
translate francais v14s07_db7ac4d7:

    # ro "Maybe not..."
    ro "Peut-être pas..."

# game/v14/scene7.rpy:108
translate francais v14s07_c23f4135:

    # ro "I now invite you all to the bus so that we can get on our way to the airport."
    ro "Je vous invite tous maintenant à rejoindre le bus pour que nous puissions nous diriger vers l'aéroport."
